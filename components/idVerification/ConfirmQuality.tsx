...
const ConfirmQuality = ({handleScreenChange, finalData} : idVerifyProps) => {
    ...
    const cropArea = finalData?.type === "Selfie" ? {
            cropX: 0.6,
            cropY: 0,
            cropWidth: 0.8,
            cropHeight: 1.5 
    ...
    const cropImageByUri = async (imageUri: string, cropArea: { cropX: number, cropY: number, cropWidth: number, cropHeight: number }) => {
    ...
    useEffect(() => {
        const cropImage = async () => {
            ...
            // Handle back image cropping (if exists)
    ...
    const handleContinue = () => {
        // Call API Send cropped photos to server !

        // Go back to start (temporary setting category as succuss)
        ...
        <MyLinearGradient className='flex-1' type="background" color={appliedTheme === 'dark' ? 'dark' : 'light-blue'}>
            <Box className="flex-1 justify-between">
                ...
                <CardUpRounded variant='card' className='justify-between items-center'>
                    {/* Photo Image with loading state */}
                    <Box 
                    ...
                    >
                        {isLoading ? (
                            ...
                            <>
                                <Image 
                                    source={croppedFrontImage ? { uri: croppedFrontImage } : finalData?.frontIdPhoto ? { uri: finalData.frontIdPhoto as string } : require('@/assets/images/something-went-wrong.png')}
                                    alt='id image'
                                    className={`rounded-xl border border-gray-400 w-full object-cover ${finalData?.type === "ID Card" ? " h-1/2": "h-full"}`}
                                />
                                {finalData?.type === "ID Card" && (
                                    <Image 
                                        source={croppedBackImage ? { uri: croppedBackImage } : finalData?.backIdPhoto ? { uri: finalData.backIdPhoto as string } : require('@/assets/images/something-went-wrong.png')}
                                        alt='id image'
                    ...
                    <Box className='items-center gap-4'>
                        ...
                        <Text className={`text-subText-${appliedTheme} text-center text-[16px]`}>
                            ...
                            "Please make sure your whole face is in the picture with no blur or glare"
                            :
                    ...
                    <Box className='items-center gap-6 mb-10 w-[93%]'>
                        <MyLinearGradient type='button' color='purple'>
                            <Button onPress={handleContinue}>
                                <ButtonText className='text-center w-full font-semibold text-lg'>Continue</ButtonText>
                            </Button>
                        </MyLinearGradient>
