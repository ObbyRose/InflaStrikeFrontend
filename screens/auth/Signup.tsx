...
const Signup: React.FC<Props> = ({ navigation }) => {
  ...
  const handleScreenChange = (
    newScreenStep: 'back' | 'next' | string,
    data?: any,
    fromEdit = false
  ) => {
    ...
    const isScreenName =
      typeof newScreenStep === 'string' && !['back', 'next'].includes(newScreenStep);
    ...
    if (fromEdit) setComeFromEdit(true);
    else setComeFromEdit(false);
    ...
    Animated.timing(slideAnim, {
      ...
      if (isScreenName) {
        ...
        // "back"
        if (screenStep === 'PHONE_NUMBER') navigation.navigate('Login');
        else setScreenStep(screens[currentIndex - 1]);
  ...
  function chooseHeaderStep(newScreenStep: string) {
    if (screenStep === 'CREATE_PASSWORD') setHeaderStep(1);
    else if (screenStep === '') setHeaderStep(2);
    else if (screenStep === '') setHeaderStep(3);
    else setHeaderStep(null);
  }

  useEffect(() => {
    const backAction = () => {
      if (screenStep === 'PHONE_NUMBER') navigation.navigate('Login');
      else handleScreenChange('back');
      return true;
    };
          ...
          <Box className={`min-h-screen flex-1`}>
            ...
            <Box className={`flex-1 p-10 pt-5`}>
              <Animated.View
                  ...
                  transform: [
                    {
                      translateX: slideAnim.interpolate({
                        inputRange: [-1, 0, 1],
                        outputRange: [isGoingBack ? 350 : -350, 0, isGoingBack ? -350 : 350],
                      }),
                    },
                  ],
                ...
                {screenStep === 'PHONE_NUMBER' && (
                  <SignupPhoneNumber handleScreenChange={handleScreenChange} />
                )}
                {screenStep === 'VERIFY_PHONE' && (
                  <SignupVerifyPhone
                    handleScreenChange={handleScreenChange}
                    phoneEntered={finalData?.phoneNumber || 'your phone'}
                  />
                )}
                {screenStep === 'CREATE_PASSWORD' && (
                  <SignupCreatePassword
                    handleScreenChange={handleScreenChange}
                    phoneEntered={finalData?.phoneNumber || 'your phone'}
                  />
                )}
                {screenStep === 'PERSONAL_INFO' && (
